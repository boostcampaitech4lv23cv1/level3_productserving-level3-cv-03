...
ckpt = 'faster_rcnn_r50_fpn_1x_coco_20200130-047c8118.pth'  # checkpoint in mmdetection/demo/inference_demo.ipynb
...
@router.post('/OD', tags=['OD'], description='사진 업로드 요청')
async def load_image(files : UploadFile = File(...)):
    img = Image.open(BytesIO(await files.read())).convert('RGB')
